...
public class DependentService : IDependentService
{
    ...
    {
        var dependents = await _dependentRepository.FindByAsNotTraking(x => x.DependentId == id).FirstOrDefaultAsync();
        var response = _mapper.Map<DependentResponseDto>(dependents);
        return response;
    }
    ...
    {
        var dependents = await _dependentRepository.FindByAsNotTraking(x => x.UserId == userId).ToListAsync();
        var response = _mapper.Map<IEnumerable<DependentResponseDto>>(dependents);
        return response;
    }
